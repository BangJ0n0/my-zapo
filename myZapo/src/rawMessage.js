...
const blobTableExists = !!db.prepare(`
    SELECT name
    FROM sqlite_master
    WHERE type='table'
    AND name='raw_messages_blob'
`).get()
...
if (!blobTableExists) {
    ...
    PRAGMA table_info(raw_messages_blob)
`).all().some(c => c.name === 'raw_bin')) {
    db.transaction(() => {
        db.exec(`
        ...
        db.exec(`
            INSERT INTO raw_messages_blob_new (msg_id, raw_json)
            SELECT msg_id, raw_json
            FROM raw_messages_blob
        `)

        db.exec(`
            DROP TABLE raw_messages_blob
        `)

        db.exec(`
            ALTER TABLE raw_messages_blob_new
            RENAME TO raw_messages_blob
        `)
    })()
}
...
db.exec(`
    ...
    ON raw_messages(chat_id, sender_jid)
`)
...
const stmtInsertChat = db.prepare(`
    INSERT OR IGNORE INTO chats (jid)
    VALUES (?)
`)

const stmtSelectChat = db.prepare(`
    SELECT id
    FROM chats
    WHERE jid = ?
`)
...
const stmtInsertBlob = db.prepare(`
    INSERT OR IGNORE INTO raw_messages_blob
    (
        msg_id,
        raw_bin
    )
...
const stmtGetByOrder = db.prepare(`
    ...
    JOIN chats c
        ON rm.chat_id = c.id
    LEFT JOIN raw_messages_blob b
        ON rm.msg_id = b.msg_id
    WHERE rm.id = ?
`)
...
const stmtGetMediaWithRaw = db.prepare(`
    SELECT
        rm.*,
        c.jid,
        b.raw_bin,
...
const stmtPruneBlob = db.prepare(`
    DELETE FROM raw_messages_blob
    WHERE msg_id IN (
        SELECT msg_id
        FROM raw_messages
        WHERE timestamp < ?
...
const stmtPruneMeta = db.prepare(`
...
function stripDeepQuotes(value, quoteDepth = 0) {
    ...
    if (Array.isArray(value)) {
        for (const item of value) {
            stripDeepQuotes(item, quoteDepth)
        }
...
function encodeRawEvent(event) {
    try {
        ...
        if (payload.message) {
            stripDeepQuotes(payload.message)
        }
...
export function saveRawMessage(m) {
    ...
    try {
        ...
        if (changes === 0) {
            console.log(
                chalk.yellow(
                    `[DB] ${m.id} sudah pernah tersimpan, skip.`
                )
            )
        ...
        console.log(
            chalk.green(
                `[DB] ${m.id} tersimpan, order=${insertedId}`
            )
        )
...
export function getMessageByOrder(orderNumber) {
    const row =
        stmtGetByOrder.get(orderNumber)
...
export function getMessagesByJid(
    ...
    return stmtGetByJid.all(
        chatId,
        limit
    )
}

export function getMessagesByChatWithRaw(
    jid,
    limit = 100
) {
    const chatId =
...
export function getMessagesByChatWithRawAsc(
    jid,
    limit = 100
) {
    const chatId =
...
export function getMessagesBySenderWithRaw(
    jid,
    senderJid,
    limit = 100
) {
...
export function getMessagesBySenderWithRawAsc(
    jid,
    senderJid,
    limit = 100
) {
...
export function getLastActivePerSender(jid) {
    ...
    return stmtLastActivePerSender.all(
        chatId
    )
}
...
function getAlbumAssociationFromMessage(message) {
    if (
        !message ||
    ...
    const walk = (obj) => {
        ...
        const association =
            obj?.messageContextInfo?.messageAssociation ||
            obj?.contextInfo?.messageAssociation ||
            obj?.messageAssociation
...
function getAlbumParentIdFromMessage(message) {
    const association =
        getAlbumAssociationFromMessage(message)
...
export function getRawMessagesByParentId(
    parentId,
    chatJid = null
) {
    if (!parentId) return []
    ...
    for (const row of rows) {
        ...
        const mediaType =
            getMediaTypeFromMessage(message)
        ...
        const foundParentId =
            getAlbumParentIdFromMessage(message)
