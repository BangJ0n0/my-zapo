...
db.exec(`
    CREATE TABLE IF NOT EXISTS trusted_prefix_features (
        jid TEXT NOT NULL,
        command TEXT NOT NULL,
        added_by TEXT,
        created_at INTEGER NOT NULL DEFAULT (unixepoch()),
...
const stmtLoadAllPrefixTrust = db.prepare(`
    SELECT jid, command
    FROM trusted_prefix_features
`)

const stmtAddPrefixTrust = db.prepare(`
    INSERT OR IGNORE INTO trusted_prefix_features (
        jid,
        command,
        added_by
    )
...
const stmtRemovePrefixTrust = db.prepare(`
    DELETE FROM trusted_prefix_features
    WHERE jid = ? AND command = ?
`)

const stmtRemovePrefixTrustJid = db.prepare(`
    DELETE FROM trusted_prefix_features
    WHERE jid = ?
`)

const prefixTrustCache = new Map()

for (const { jid, command } of stmtLoadAllPrefixTrust.all()) {
    if (!prefixTrustCache.has(jid)) {
        prefixTrustCache.set(jid, new Set())
    }
...
export function isTrustedPrefixFeature(jid, command) {
    if (!jid || !command) {
        return false
    }

    return prefixTrustCache
        .get(jid)
        ?.has(command.toLowerCase()) ?? false
}

export function addTrustedPrefixFeature(
    jid,
    command,
    addedBy = null
) {
    ...
    const added = stmtAddPrefixTrust
        .run(jid, command, addedBy)
        .changes > 0
...
export function removeTrustedPrefixFeature(
    jid,
    command
) {
    if (!jid || !command) {
    ...
    const removed = stmtRemovePrefixTrust
        .run(jid, command)
        .changes > 0

    const commands = prefixTrustCache.get(jid)

    if (commands) {
        commands.delete(command)

        if (!commands.size) {
            prefixTrustCache.delete(jid)
        }
    }
...
export function removePrefixTrust(jid) {
    if (!jid) {
        return false
    }

    const removed = stmtRemovePrefixTrustJid
        .run(jid)
        .changes > 0
...
export function getTrustedPrefixFeatures() {
    return prefixTrustCache
}