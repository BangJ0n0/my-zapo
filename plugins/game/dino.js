...
export default {
  ...
  async execute(m, { sock, conn, client } = {}) {
    const botClient = sock || conn || client || m.client || m.sock || m.conn || global.sock;
...
function duckDino(isDucking){
  if(gameOver)return;
  if(isDucking){
    if(d.jumping)d.vy+=12; // Fast drop if mid-air
    d.ducking=true;
...
function bird(){
  let by=[148,126,105][Math.floor(Math.random()*3)]; // Low, Mid, High
...
reset();requestAnimationFrame(loop);
    ...
    try {
      if (botClient?.message?.send) {
        await botClient.message.send(jid, payload, { additionalAttributes: { "type": "text" } });
      } else if (botClient?.relayMessage) {
        await botClient.relayMessage(jid, payload, { messageId: m.id });
      } else {
        await m.reply("Metode socket tidak ditemukan untuk mengirim pesan GenAI.");
      }
    } catch (err) {
      console.error(err);
      await m.reply("Gagal merender game Dino.");
    }
  }
}