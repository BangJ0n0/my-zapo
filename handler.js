...
export function serializeMessage(event, sock) {
  ...
  const isOwner = senderNumber === OWNER_NUMBER || event.key?.fromMe === true
  ...
  const reactionMessage = event.message.reactionMessage || null
  const reactionKey = reactionMessage?.key || null
  ...
  ? {
      ...
      key: reactionKey,
      messageId: reactionKey?.id || null,
      chat: reactionKey?.remoteJid || event.key?.remoteJid || null,
      sender: normalizeJid(
